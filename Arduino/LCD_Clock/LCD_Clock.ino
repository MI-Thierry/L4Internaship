...
public:
  ...
  void Update() {
    _timeManager->Tick();
    if (digitalRead(_modeButton) == LOW) {
      _mode++;
      if (_mode > Mode::SetTime)
        _mode = Mode::Default;
    } else if (digitalRead(_resetButton) == LOW && _mode != Mode::ShowTime) {
      _subMode++;
      if (_subMode > TimeSetContext::Minute)
        _subMode = TimeSetContext::Hour;
    } else if (digitalRead(_startButton) == LOW && _mode != Mode::ShowTime) {
    ...
    } else if (digitalRead(_lightButton) == LOW) {
      _lcd.backlight();
    }
  }
...
void setup() {
  timeManager.Init(SecondElapsed, 500);
  ui.Init();
}
